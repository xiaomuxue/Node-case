...
function userzc(){
    ...
    $.post("userRegister",{uname:uname,pwd:pwd,pwdagain:pwdagain},function(data){
       data= $.trim(data);
        switch(data){
            case "1":$("#registertishi").text("用户名不能为空.....");
                break;
            case "2":$("#registertishi").text("密码不能为空.....");
                break;
            case "3":$("#registertishi").text("两次密码输入不一致.....");
                break;
            case "4":$("#registertishi").text("数据库连接失败.....");
                break;
            case "5":$("#registertishi").text("数据添加失败.....");
                break;
            case "6":$("#registertishi").text("注册成功.....");
                break;
            default:$("#registertishi").text("注册失败.....");
                break;
        }
    },"text");
}
...
function userLogin(){
    ...
    $.post("userLogin",{uname:uname,pwd:pwd},function(data){
        ...
        switch(data){
            ...
            case "3":alert("数据库连接失败......");break;      //真实的项目最好不要使用alert
...
function checkInfos(obj,tabName,colName){    //信息校验
    var info=obj.value;
    if(info!=""){
        //发送请求到服务器看该用户是否已经被注册
        $.get("checkUserName",{uname:info,tabName:tabName,colName:colName},function(data){
            ...
            if(data=="0"){
                ...
                $("#zcunamep").text("用户名校验成功.....").css("color","green");
            }else{
                $(obj).css("border-color","red");
                $("#zcunamep").text("该用户名已被占用.....");
            }
        });
    }else{
        $(obj).css("border-color","red");
